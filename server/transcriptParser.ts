...
export function processTranscriptLine(
	...
	try {
		...
		// Extract top-level metadata (present on ALL record types for every agent)
		...
		if (recordTeamName && recordTeamName !== agent.teamName) {
			...
			sink?.postMessage({ type: 'agentMeta', id: agentId, teamName: recordTeamName });
		}
		const recordGitBranch = record.gitBranch as string | undefined;
		if (recordGitBranch && recordGitBranch !== agent.gitBranch) {
			agent.gitBranch = recordGitBranch;
			sink?.postMessage({ type: 'agentMeta', id: agentId, gitBranch: recordGitBranch });
		}
			...
			if (hasToolUse) {
				...
				for (const block of blocks) {
					if (block.type === 'tool_use' && block.id) {
						...
						// Detect TeamCreate → mark this agent as a team lead
						if (toolName === 'TeamCreate' && typeof input.team_name === 'string') {
							const teamName = input.team_name;
							if (teamName !== agent.teamName) {
								agent.teamName = teamName;
								sink?.postMessage({ type: 'agentMeta', id: agentId, teamName });
							}
						}
						const agentName = toolName === 'Task' && typeof input.name === 'string'
							? input.name as string : undefined;
						const teamName = toolName === 'Task' && typeof input.team_name === 'string'
							? input.team_name as string : undefined;
						// If a Task spawns into a team, also mark the parent as team lead
						if (teamName && teamName !== agent.teamName) {
							agent.teamName = teamName;
							sink?.postMessage({ type: 'agentMeta', id: agentId, teamName });
						}
						sink?.postMessage({
