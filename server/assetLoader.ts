...
function pngToSpriteData(pngBuffer: Buffer, width: number, height: number): string[][] {
	try {
		...
		if (png.width !== width || png.height !== height) {
			console.warn(`PNG dimensions mismatch: expected ${width}x${height}, got ${png.width}x${png.height}`);
		}
		const sprite: string[][] = [];
		const data = png.data;
